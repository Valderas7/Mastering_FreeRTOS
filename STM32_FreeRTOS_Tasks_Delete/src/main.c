...
int main(void)
{
	...
	sprintf(usr_msg,"This is Demo of Task Delete APIs\r\n");
	printmsg(usr_msg);
...
void vTask1_handler(void *params)
{
	...
	while(1)
	{
		// El LED parpadea cada 200ms. *Delay bloquea la tarea 1 para que 'el idle task' se ejecute y libere la memoria de la tarea 2 eliminada*
		vTaskDelay(200);
...
void vTask2_handler(void *params)
{
	...
	while(1)
	{
		...
		{
			// Si se pulsa el boton de usuario, esta tarea se elimina
			sprintf(usr_msg, "Task-2 is about to being deleted \r\n");
			printmsg(usr_msg);
