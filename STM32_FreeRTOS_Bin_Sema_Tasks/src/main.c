...
int main(void)
{
	 ...
	 * el 'handle' creado anteriormente. Por defecto, el
	 * estado inicial del semáforo está vacío, por lo que
	 * hay que 'dar' primero el semáforo antes de 'cogerlo'. */
...
void vManagerTask(void *pvParameters)
{
	...
	for( ;; )
	{
		...
		if (xStatus != pdFALSE)
		{
			/* La tarea 1 'da' el semáforo para que la
			 * tarea 2 pueda cogerlo. */
...
void vEmployeeTask(void *pvParameters)
{
	...
	for( ;; )
	{
		/* La tarea 'coge' el semáforo. Si no lo puede
		 * 'coger', la tarea se bloquea indefinidamente. */
		...
		/* Se cede el control del procesador de nuevo a la
		 * tarea-1 porque ya se ha recibido el 'Ticket ID'. */
		taskYIELD();
	}
}
