...
void led_task_handler(void *params)
{
	...
	while(1)
	{
		// Wait until receive any notification event from button_task
		if (xTaskNotifyWait(0, 0, &current_notification_value, portMAX_DELAY) == pdTRUE)
		{
			// Notification received, so toggle the LED
...
void button_task_handler(void *params)
{
	while(1)
	{
		...
		{
			// Wait 200ms to compensate the button debouncing
			rtos_delay(200);

			// The button is pressed so let's send the notification to led_task
