...
/* Usado como contador de bucle para crear
 * un delay. */
...
int main(void)
{
	...
	sprintf(usr_msg,"\r\nEsta es la Demo de 'Exclusión mutua usando semáforos binarios'.\r\n");
	printmsg(usr_msg);

	/* Se crea un semáforo binario, pasando como argumento
	 * el 'handle' de semáforo creado anteriormente. */
	...
	{
		/* Se crea la Tarea '1'. */
		xTaskCreate(vTask1, "Task-1", 500, NULL, 1, &xTaskHandle2);

		/* Se crea la Tarea '2'. */
		xTaskCreate(vTask2, "Task-2", 500, NULL, 1, &xTaskHandle1);
